"""3D Variational Autoencoder for Minecraft house generation.
...
class Decoder3D(nn.Module):
    ...
    def __init__(self, latent_dim: int = 256):
        ...
        # We'll use adaptive padding in forward
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        # Adjust Y dimension from 32 to 42
        # Pad with zeros to match target size
        if x.size(3) != 42:
            pad_y = 42 - x.size(3)
            x = F.pad(x, (0, 0, 0, pad_y, 0, 0))
